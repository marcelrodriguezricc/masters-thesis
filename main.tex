...
\par{The first active sensor we will discuss is RADAR (Radio Detection and Ranging), a system that transmits packets in the microwave portion of the electromagnetic spectrum, receives the reflection of the emitted signal off of objects in its surroundings, then measures the delay time to ascertain the distance of reflective surfaces from the source.~\textsuperscript{[61]}~\textsuperscript{[62]} Commonly, information from RADAR can be extended to allow for the detection, location, and tracking of objects. The first application of RADAR was as a aircraft detection and warning system pioneered during the Second World War, but it's role has since expanded to include ecological, meterological, and oceanographical applications.~\textsuperscript{[48]}~\textsuperscript{[61]}~\textsuperscript{[62]}}

\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{images/radar.png}
    \caption{Placeholder for self generated version of this image from Radar Ornithology.~\textsuperscript{[60]}}
    \label{figure16}
\end{figure}
...
% LiDAR
% Altimeter / Sounder
% Active & Passive Sonar (Single beam, multibeam, side scan, 3D)
