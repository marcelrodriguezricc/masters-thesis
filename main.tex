...
\par{In the mid-twentieth century, NASA sponsored studies toward applications for multispectral imaging in a leadup to the creation of the Landsat program in 1967.~\textsuperscript{[46]}~\textsuperscript{[48]}~\textsuperscript{[50]} Landsat is a joint NASA / USGS program to monitor terrestrial land use from a satellite, the first iteration of which was launched in 1972, and has since captured millions of images of the earth's surface, providing a continuous record of Earth's land for over half a century.~\textsuperscript{[45]}~\textsuperscript{[46]}~\textsuperscript{[51]} As mentioned earlier, Landsat represents one of the most robust and comprehensive remote sensing databases and enables research across multiple fields, but does not meet the spatial or temporal resolution required to characterize some of the most essential processes at the focus of coastal science. Additionally, previous iterations of Landsat oriented toward land-use applications, in order meet necessary constraints due to technological restrictions, excluded bands capable of gathering information about aquatic bodies.~\textsuperscript{[55]}~\textsuperscript{[56]} However, the most recent iterations, Landsat 8 and 9 include technologies that are able to monitor important biogeochemical properties of coastal waters, which we will visit in depth later on.~\textsuperscript{[53]}~\textsuperscript{[54]} Regardless, the Landsat program is especially worth mentioning here due to the program's historic role in launching a revolution in electronic imaging technologies, and because it continues to offer openly and easily accessible information on the most cutting-edge and rigorously developed remote sensing practices and technologies.}

\par{Multispectral imaging sensors are able to collect data from non-visable bands of the electromagnetic spectrum---currently, imaging sensors exist for bands of frequencies both below and above that of visible light, such as microwave, infrared, and ulraviolet.~\textsuperscript{[48]} These bands can be divided further into constituent sub-bands to extract information about different phenomena relevant to environmental research.~\textsuperscript{[45]}~\textsuperscript{[48]} For example, the infrared band (0.7-300μm) contains valuable information pertaining to a large range of environmental phenomena, and can be divided into the following categories---Near Infrared (NIR, 0.7-1.5µm) used for measuring vegetation health and biomass coverage, Short Wavelength Infrared (SWIR, 1.5-3µm) used to measure soil moisture and detect minerals, Mid Wavelength Infrared (MWIR, 3-8µm) used for panchromatic sharpening and cloud screening, and Long Wavelength Infrared (LWIR, 8-15µm) used to measure surface temperatures, and Far Infrared (FIR, greater than 15µm) used for atmospheric profiling and ice characterization.~\textsuperscript{[45]}~\textsuperscript{[46]}~\textsuperscript{[51]}~\textsuperscript{[52]} Waves from some bands are able to penetrate through clines and layers such as the surface of the ocean and the atmosphere of the Earth, while others are not.}

\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{images/landsat-8-specs.png}
    \caption{Placeholder for self generated version of this chart from NASA Landsat 8.}
    \label{figure13}
\end{figure}

\par{Aquatic environments pose another problem: water absorbs and scatters light, which is especially true waters that are highly turbid, as are the waters in middle-to-high latitude coastal areas.~\textsuperscript{[55]}~\textsuperscript{[56]} Especially pertinent to our scope are new developments with the Optical Land Imager device mounted on the most recent Landsat 8 and 9 satellites, which is able to capture low-frequency information from the blue / violet section of the visible spectrum. For purposes of the Landsat program, this band is titled Coastal / Aerosol Band 1, as per it's primary usage, which is to monitor shallow water areas for coloring agents---chlorophyll, total suspended solids (TSS), and yellowing colored dissolved organic materials (CDOM).~\textsuperscript{[46]}~\textsuperscript{[53]}~\textsuperscript{[54]}~\textsuperscript{[55]}  }
