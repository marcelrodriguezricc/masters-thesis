...
\par{Another ranging technique that predates the use electromagnetic radiation is Sound Navigation and Ranging (SONAR), used to measure the distance of objects relative to the source acoustically by the propagation of sound waves through a medium (usually water). Acoustic sensing techniques are particularly effective for underwater applications such as seafloor mapping because sound waves experience less attenuation in water compared to electromagnetic radiation, allowing for more accurate detection over longer distances.~\textsuperscript{[77]} Sonar technologies may either actively emit sound waves to reflect off of surrounding surfaces and measure the time-of-flight to ascertain distance, or passively detect sounds emitted by other sources.~\textsuperscript{[77]} Sound waves are converted into electrical signals through transducers, that are then subjected to processing techniques to derive further information, including the distance and trajectory of a moving object. Like RADAR, with active Sonar, information on the material properties of surfaces is available through spectral analysis backscatter measurement.~\textsuperscript{[78]}}
...
% Sonar systems exist in various configurations based on use-case, }
% For accurate observation of the seafloor, physical properties of the ocean must be considered.
% Often, Sonar is deployed from a surface vessel or a submerisble. Tradeoffs between distances from seafloor resolution vs footprint. AUVS are also used. Acoustic navigation. Ultrashort baseline.
...
\begin{enumerate}
    ...
    % 78
    \item{“Multispectral multibeam echosounder backscatter data.” Accessed: Jun. 01, 2025. [Online]. \href{https://www.hydro-international.com/content/article/multispectral-multibeam-echosounder-backscatter-data?utm_source=chatgpt.com}{Available}}
